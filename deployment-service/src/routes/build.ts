...
buildRouter.post("/create/v2", asyncHandler(async (req: Request, res: Response) => {
    ...
    const prevBuilds = await client.query("SELECT * FROM builds WHERE project_id = $1", [deploymentId]);
    ...
    const container = await docker.createContainer({
      ...
      const buildData = await client.query(`INSERT INTO builds (status, status_message,build_started_at, 
        commit_hash, commit_message,project_id, container_id)
      ...
      await client.query(`UPDATE projects SET last_build_id = $1, last_deployed_hash = $2, last_deployed_at = $3 WHERE id = $4`, [buildData.rows[0].id, to_deploy_commit_hash, new Date(), deploymentId]);
      const randomString = Math.random().toString(36).substring(2, 9);
      await client.query(`INSERT INTO domains (sub_domain, project_id, ip, port) VALUES($1, $2, $3, $4)`, [randomString, deploymentId, "localhost", port]);
